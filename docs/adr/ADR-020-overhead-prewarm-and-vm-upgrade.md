...
- **Memory bumped 512mb → 1024mb after first deploy.** First TLE refresh on the new VM OOM-killed the machine starting at the 13th feed (planet): SATCAT 68k cache (~34 MB) + per-feed OMM parse + 16k Satrec objects + warm_positions_cache running on a 60s timer stacked above the 512 MB limit. Doubling memory adds ~$1-2/month and removes the OOM cliff without changing the ingest path.
